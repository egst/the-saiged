...
export default class ImageBreakSection extends Section {
    ...
    async #pick () {
        ...
        // Best-effort variant pre-generation — intentionally not awaited so it
        // doesn't delay showing the spinner after the picker closes.
    ...
    #syncPickState (trackLoad = false) {
        ...
        if (trackLoad) {
            ...
            // Two rAFs: first rAF triggers a paint with spinner visible, second rAF sets
            // src. Without the double-rAF, cached images fire `load` in microtasks within
            // the same rendering step, removing the spinner before the first paint.
